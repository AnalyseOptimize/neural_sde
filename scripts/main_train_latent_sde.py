...
from src.train import LatentSDETrainConfig, train_latent_sde
...
def maybe_save_plots(
    *,
    ...
    fig = plot_epoch_diagnostics(
        history,
        option="loss",
        save_path=fig_dir / "latent_sde_loss_by_epoch.pdf",
    )
    plt.close(fig)
    ...
    with torch.no_grad():
        ...
        generated = model.sample_paths(
            ts.to(device=device),
            y0,
            method=cfg.train.sample_method,
            dt=float(cfg.train.sample_dt),
        ).detach().cpu()
...
@hydra.main(version_base=None, config_path="../configs", config_name="latent_sde")
def main(cfg: DictConfig) -> dict[str, list[float]]:
    ...
    run_logger.info(
        "Latent SDE prepared: params={}, data_size={}, latent_size={}, context_size={}, hidden_size={}",
    ...
    if bool(cfg.evaluation.get("coupled_brownian", False)):
        run_logger.warning(
            "evaluation.coupled_brownian is ignored for latent_sde: "
            "the latent SDE Brownian motion is not the simulator Brownian driver."
        )

    train_config = LatentSDETrainConfig(
        epochs=int(cfg.train.epochs),
        steps=cfg.train.steps,
        steps_per_epoch=cfg.train.steps_per_epoch,
        batch_size=int(cfg.train.batch_size),
        lr=float(cfg.train.lr),
        lr_gamma=float(cfg.train.lr_gamma),
        weight_decay=float(cfg.train.weight_decay),
        kl_anneal_iters=int(cfg.train.kl_anneal_iters),
        noise_std=float(cfg.train.noise_std),
        adjoint=bool(cfg.train.adjoint),
        method=str(cfg.train.method),
        dt=float(cfg.train.dt),
        sample_method=cfg.train.sample_method,
        sample_dt=float(cfg.train.sample_dt),
